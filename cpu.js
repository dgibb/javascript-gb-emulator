...
//0x28
 jr_z_n : function(){
	if (cpu.zeroFlag()){
		cpu.cpu.pc+=cpu.signDecode(memory.readByte(cpu.pc+1));
		cpu.t=12; cpu.m=0;
		}
	else{
		cpu.m=2; cpu.t=8;
...
//0x2A
 ldi_hl : function(){
	cpu.a=readByte(cpu.getAddr(cpu.h,cpu.l));
	cpu.l++;
	cpu.l&=0xFF;
	if (cpu.l===0){
		cpu.h++;
...
//0x35
 dec_hl_ : function(){
	 data = readByte(cpu.getAddr(cpu.h,cpu.l))
	datcpu.a--;
	memory.writeByte(data, addr);
	if (datcpu.a===0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
...
//0x36
 ld_hl_n : function(){
	writeByte(cpu.getAddr(cpu.h,cpu.l), memory.readByte(cpu.pc+1) );
	cpu.m=2; cpu.t=12;
},
...
//0x70
 ld_hl_b: function(){
	memory.writeByte(cpu.getAddr(cpu.h,cpu.l),b);
	cpu.m=1; cpu.t=4;
},

//0x71
 ld_hl_c: function(){
	memory.writeByte(cpu.getAddr(cpu.h,cpu.l),cpu.c);
	cpu.m=1; cpu.t=4;
},

//0x72
 ld_hl_d: function(){
	memory.writeByte(cpu.getAddr(cpu.h,cpu.l),d);
	cpu.m=1; cpu.t=4;
},

//0x73
 ld_hl_e: function(){
	memory.writeByte(cpu.getAddr(cpu.h,cpu.l),e);
	cpu.m=1; cpu.t=4;
},

//0x74
 ld_hl_h: function(){
	memory.writeByte(cpu.getAddr(cpu.h,cpu.l),h);
	cpu.m=1; cpu.t=4;
},
...
//0x86
 add_a_hl : function(){
	var value = readByte(cpu.getAddr(cpu.h,cpu.l));
...
//0xB8
 cp_b : function(){
	var result= cpu.a-cpu.b;
	if (result < 0){cpu.setCarryFlag();}
	if(cpu.a&0x0F < b&0x0F){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (result === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.setSubFlag();
	cpu.m=1; cpu.t=4;
},
//0xB9
 cp_c : function(){
	var result= cpu.a-cpu.c;
	if (result < 0){cpu.setCarryFlag();}
	if(cpu.a&0x0F < c&0x0F){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (result === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.setSubFlag();
	cpu.m=1; cpu.t=4;
},
...
//0xBA
 cp_d : function(){
	var result= cpu.a-cpu.d;
	if (result < 0){cpu.setCarryFlag();}
	if(cpu.a&0x0F < d&0x0F){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (result === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.setSubFlag();
	cpu.m=1; cpu.t=4;
},
...
//0xBB
 cp_e : function(){
	 result= cpu.a-cpu.e;
	if (result < 0){cpu.setCarryFlag();}
	if(cpu.a&0x0F < e&0x0F){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (result === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.setSubFlag();
	cpu.m=1; cpu.t=4;
},
...
//0xBC
 cp_h : function(){
	 result= cpu.a-cpu.h;
	if (result < 0){cpu.setCarryFlag();}
	if(cpu.a&0x0F < h&0x0F){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (result === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.setSubFlag();
	cpu.m=1; cpu.t=4;
},
...
//0xBD
 cp_l : function(){
	 result= cpu.a-cpu.l;
	if (result < 0){cpu.setCarryFlag();}
	if(cpu.a&0x0F < l&0x0F){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (result === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.setSubFlag();
	cpu.m=1; cpu.t=4;
},
...
//0xBF
 cp_a : function(){
	var result= cpu.a-cpu.a;
	if (result < 0){cpu.setCarryFlag();}
	if(cpu.a&0x0F < a&0x0F){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (result === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.setSubFlag();
	cpu.m=1; cpu.t=4;
},
...
//0xE8
 add_sp_d : function(){
	...
	cpu.resetZeroFlag();
	cpu.resetSubFlag();
	if(((cpu.sp&0x0F) + (value&0x0F))&0x10){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if (cpu.sp>65535){cpu.setCarryFlag();} else {cpu.resetCarryFlag();}
	cpu.m=2; cpu.t=16;
},

//0xE9
 jp_hl : function(){
	cpu.pc=memory.readByte(cpu.getAddr(cpu.h,cpu.l));
	cpu.m=1; cpu.t=4;
},
...
//0xFB
 ld_a_nn : function(){
	cpu.a=memory.readWord(cpu.pc+1);
	cpu.m=3; cpu.t=16;
},
...
//0x10
 rl_b : function(){
	var carry = cpu.carryFlag()?1:0;
	if (cpu.b&0x80===0x80){cpu.setCarryFlag();} else {cpu.resetCarryFlag();}
	cpu.b=cpu.b<<1;
	cpu.b+=carry;
	cpu.b&=0xFF;
	if(cpu.b===0){cpu.setZeroFlag();} else {cpu.resetZeroFlag();}
...
//0x20
 sla_b : function(){
	if(cpu.b&0x80){cpu.setCarryBit();} else {cpu.resetCarryBit();}
	cpu.b=cpu.b<<1;
	cpu.b&=0xFF;
	if(cpu.b===0){cpu.setZeroFlag();} else {cpu.resetZeroFlag();}
	cpu.resetHalfFlag();
...
//0x21
 sla_c : function(){
	if(cpu.c&0x80){cpu.setCarryBit();} else {cpu.resetCarryBit();}
	cpu.c=cpu.c<<1;
	cpu.c&=0xFF;
	if(cpu.c===0){cpu.setZeroFlag();} else {cpu.resetZeroFlag();}
	cpu.resetHalfFlag();
...
//0x22
 sla_d : function(){
	if(cpu.d&0x80){cpu.setCarryBit();} else {cpu.resetCarryBit();}
	cpu.d=cpu.d<<1;
	cpu.d&=0xFF;
	if(cpu.d===0){cpu.setZeroFlag();} else {cpu.resetZeroFlag();}
	cpu.resetHalfFlag();
...
//0x23
 sla_e : function(){
	if(cpu.e&0x80){cpu.setCarryBit();} else {cpu.resetCarryBit();}
	cpu.e=cpu.e<<1;
	cpu.e&=0xFF;
	if(cpu.e===0){cpu.setZeroFlag();} else {cpu.resetZeroFlag();}
	cpu.resetHalfFlag();
...
//0x25
 sla_l : function(){
	if(cpu.h&0x80){cpu.setCarryBit();} else {cpu.resetCarryBit();}
	cpu.l=cpu.l<<1;
	cpu.l&=0xFF;
	if(cpu.l===0){cpu.setZeroFlag();} else {cpu.resetZeroFlag();}
	cpu.resetHalfFlag();
...
//0x26
 sla_hl : function(){
	var value=memory.readByte(cpu.getAddr(cpu.h,cpu.l));
	if(value&0x80){cpu.setCarryBit();} else {cpu.resetCarryBit();}
	value<<1;
	value&=0xFF;
	memory.writeByte(value, cpu.getAddr(cpu.h,cpu.l));
	if(value===0){cpu.setZeroFlag();} else {cpu.resetZeroFlag();}
...
//0x56
 bit_2_hl : function() {
	var value=memory.readByte(cpu.getAddr(cpu.h,cpu.l));
	if(cpu.value&0x04){cpu.resetZeroFlag();} else {cpu.setZeroFlag();}
	cpu.resetSubFlag();
	cpu.setHalfFlag();
	cpu.m=2; cpu.t=16;
}, 
