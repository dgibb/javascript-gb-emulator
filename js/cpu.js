...
//0x19
 add_hl_de : function(){
	...
	cpu.h=z>>8;
	if ((x&0x0F00 + y&0x0F00)&0x1000){cpu.setHalfFlag();} else {cpu.resetHalfFlag();}
	if (z>0xFFFF){cpu.setCarryFlag();cpu.h&=0xFFFF;} else {cpu.resetCarryFlag();}
	cpu.resetSubFlag();
	cpu.m=1; cpu.t=8;
},
...
//0x36
 ld_hl_n : function(){
	memory.writeByte(cpu.getAddr(cpu.h,cpu.l), memory.readByte(cpu.pc+1) );
	cpu.m=2; cpu.t=12;
},
...
//0xCE
 adc_a_n : function(){
	...
	if (result & 0xFF00){cpu.setCarryFlag();}
	if(((cpu.a&0x0F) + (cpu.l&0x0F))&0x10){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	if ((result&0x00FF) === 0){cpu.setZeroFlag();}else{cpu.resetZeroFlag();}
	cpu.a=(result&0x00ff);
	cpu.resetSubFlag();
	cpu.m=2; cpu.t=8;
},
...
//0xF8
 ldhl_sp_d : function(){
	...
	if(((cpu.sp&0x0F00) + (value&0x0F00))&0x1000){cpu.setHalfFlag();}else {cpu.resetHalfFlag();}
	cpu.resetZeroFlag();
	cpu.resetSubFlag();
	cpu.m=2; cpu.t=12;
},
...
executeBios: function(){
	cpu.timer = window.setInterval(cpu.runFrame,17);
	//MEMORY.splice(0,256);
