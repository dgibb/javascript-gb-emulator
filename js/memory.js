//David Gibb

memory = {
	
mbcr1:0,
mbcr2:0,
mbcr3:0,
mbcr4:0,
byteArray:0,
ramSize:0,
romSize:8196,


//functions

readByte: function(addr){
	
	switch (addr&0xF000){
	
	case 0x4000:
	case 0x5000:
	case 0x6000:
	case 0x7000:
	
	if(!memory.mbcr4){
		return romBanks[memory.mbcr3>>6][memory.mbcr2&0x1F][addr-0x4000];
	}else{
		return romBanks[0][memory.mbcr2&0x1F][addr-0x4000];
	}
	break;

	case 0x8000:
	case 0x9000:	
		
		if(addr>=0x8000 && addr<0x9000){
			return tileSet1[addr-0x8000];
		}else if(addr>=0x9000 && addr<0x9800){
			return tileSet0[addr-0x8800];
		}else if(addr>=0x9800 && addr<0x9C00){
			addr-=0x9800;
			return tileMap0[Math.floor(addr/32)][addr%32];
		}else if(addr>=0x9C00 && addr<0xA000){
			addr-=0x9C00;
			return tileMap1[Math.floor(addr/32)][addr%32];
		}
		break;
		
	
	case 0xA000:
	case 0xB000:
	if(memory.mbcr4===0x0A){
		return ramBanks[memory.mbcr2&0x03][addr-0xA000]
	}else{
		return ramBanks[0][addr-0xA000]
	}
	break;
	
	case 0xF000:
	
		switch (addr&0xFF00){
			
			case 0xFF00:
			switch(addr){
			
				case 0xFF42:
				return display.scrollY;
				break;
			
				case 0xFF43:
				return display.scrollX;
				break;
			
				case 0xFF44:
				return display.line;
				break;
			
				default:
				return MEMORY[addr];
				break;				
			}
			
			case 0xFE00:
			
				if(addr>=0xFE00 && addr<0xFEA0){
				return spriteData[addr-0xFE00];
				}else{
					return MEMORY[addr];
				}
				break;
				
			default:
			return MEMORY[addr];
			break;
			
			}
	default:
	return MEMORY[addr];
	break;
	
	}
},
	
readWord: function(addr){
	var data=memory.readByte(addr+1)
	data=data<<8;
	data|=memory.readByte(addr);
	return data;
},
	
writeByte: function(data, addr){
	
	switch (addr&0xF000){
	
	case 0x0000:
	case 0x1000:
		memory.mbcr1=data;
		break;
		
	case 0x2000:
	case 0x3000:
		memory.mbcr2=data;
		break;
		
	case 0x4000:
	case 0x5000:
		memory.mbcr3=data;
		break;
	
	case 0x6000:
	case 0x7000:
		memory.mbcr4=data;
		break;
		
	case 0x8000:
	case 0x9000:	
		
		if(addr>=0x8000 && addr<0x8800){
			tileSet1[addr-0x8000]=data;
		}else if(addr>=0x8800 && addr<0x9000){
			tileSet1[addr-0x8000]=data;
			tileSet0[addr-0x8800]=data;
		}else if(addr>=0x9000 && addr<0x9800){
			tileSet0[addr-0x8800]=data;
		}else if(addr>=0x9800 && addr<0x9C00){
			addr-=0x9800;
			tileMap0[Math.floor(addr/32)][addr%32]=data;
		}else if(addr>=0x9C00 && addr<0xA000){
			addr-=0x9C00;
			tileMap1[Math.floor(addr/32)][addr%32]=data;
		}
		break;
		
	case 0xA000:
	case 0xB000:
		if (memory.mcbr4){
			ramBanks[memory.mbcr3&0x03][addr-0xA000]=data
		} else{
		ramBanks[0][addr-0xA000]=data;
		}
		break;
	
	case 0xF000:
		switch (addr&0xFF00){
			
			case 0xFF00:
			switch(addr){
				
				case 0xFF42:
				display.scrollY=data;
				break;
			
				case 0xFF43:
				display.scrollX=data;
				break;
			
				case 0xFF44:
				display.line=data;
				break;
			
				case 0xFF47:
				MEMORY[0xFF47]=data;
				for (var i=0;i<4;i++){
				var ref = data&0x03;
					switch (ref){
				
						case 0:
						paletteRef[i]=255;
						break;
					
						case 1:
						paletteRef[i]=192;
						break;
					
						case 2:
						paletteRef[i]=96;
						break;
					
						case 3:
						paletteRef[i]=0;
						break;
					}
				data=data>>2;
				}
				break;
				
				default:
				MEMORY[addr]=data;
				break;
			
			}
			case 0xFE00:
			if(addr>=0xFE00 && addr<0xFEA0){
				spriteData[addr-0xFE00]=data;
			}else{
				MEMORY[addr]=data;
			}
			break;
			
			default:
			MEMORY[addr=data];
			break;
		}
		
	default:
		MEMORY[addr]=data;
		break;
	}
},
	
writeWord: function(data, addr){
	var data2=data&0xFF;
	data=data>>8;
	MEMORY[addr+1]=data;
	MEMORY[addr]=data2;	
},

loadROM: function(){
	var input = document.getElementById('romFileInput');
	var file = input.files[0];
	if (file===undefined){
		alert("Please Select a File");
	} else{
	var reader = new FileReader;
	reader.onload=function(e){
		
		var byteArray=new Uint8Array(reader.result);
		
		for (var i=0;i<0x4000;i++){
			MEMORY[i]=byteArray[i];
			console.log('placed', MEMORY[i].toString(16), 'at', i.toString(16));
		}
		
		biosPlaceholder=MEMORY.slice(0,256);
		
		for(i=0; i<256;i++){
			MEMORY[i]=BIOS[i];
		}	
		
		for (i=0;i<4;i++){
			for (var j=0; j<31;j++){
				romBanks[i].push(new Array(16384));
			}
		}
		
		for (i=16384; i<byteArray.length;i++){
			romBanks[Math.floor(i/524288)][Math.floor((i%524288)/16384)][i%16384]=byteArray[i];
		
		}
		cpu.executeBios();

		}
	reader.readAsArrayBuffer(file);
	}
},

memoryInit: function(){
	for(var i=256; i<65536;i++){
		MEMORY[i]=0;
	}
	for(var i=256; i<32768;i++){
		ramBanks[Math.floor(i/8192)][i%8192]=0;
	}
	console.log('memory initialized');
},

showStack: function(){
	console.log('STACK:')
	var i=0;
	while((cpu.sp&0xFF)!==0xFF){
		console.log((cpu.sp+i).toString(16), ': ', memory.readByte(cpu.sp+i));
		i+=2;
	}
	
}

};

BIOS = [
,
];

biosPlaceholder=[];
MEMORY=[];
romBanks=[[[0]],[[0]],[[0]],[[0]]];
ramBanks=[[0],[0],[0],[0]];